...
private fun uniffiCheckApiChecksums(lib: IntegrityCheckingUniffiLib) {
    ...
    if (lib.uniffi_mopro_bindings_checksum_func_generate_halo2_proof() != 1670.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_mopro_bindings_checksum_func_verify_circom_proof() != 14151.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_mopro_bindings_checksum_func_verify_halo2_proof() != 16112.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
}
...
fun `verifyHalo2Proof`(
    ...
    `proofData`: kotlin.ByteArray,
    `publicInputs`: kotlin.ByteArray,
): kotlin.Boolean =
    FfiConverterBoolean.lift(
        uniffiRustCallWithError(MoproException) { _status ->
            UniffiLib.INSTANCE.uniffi_mopro_bindings_fn_func_verify_halo2_proof(
                ...
                FfiConverterByteArray.lower(`proofData`),
                FfiConverterByteArray.lower(`publicInputs`),
                _status,
            )
        },
    )
