...
public func generateHalo2Proof(srsPath: String, pkPath: String, inputs: [String: [String]])throws  -> Halo2ProofResult  {
...
public func verifyHalo2Proof(srsPath: String, vkPath: String, proofData: Data, publicInputs: Data)throws  -> Bool  {
    return try  FfiConverterBool.lift(try rustCallWithError(FfiConverterTypeMoproError_lift) {
    uniffi_mopro_bindings_fn_func_verify_halo2_proof(
        FfiConverterString.lower(srsPath),
        FfiConverterString.lower(vkPath),
        FfiConverterData.lower(proofData),
        FfiConverterData.lower(publicInputs),$0
    )
})
}
...
private let initializationResult: InitializationResult = {
    ...
    if (uniffi_mopro_bindings_checksum_func_generate_halo2_proof() != 1670) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mopro_bindings_checksum_func_verify_circom_proof() != 14151) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_mopro_bindings_checksum_func_verify_halo2_proof() != 16112) {
        return InitializationResult.apiChecksumMismatch
    }
